...
function App() {
  ...
  // Stable ScrollTrigger sync — only registers once
  useLenis(
    (lenisInstance) => {
  ...
  // Disable all scroll animations if reduced motion is preferred
  React.useEffect(() => {
...
function FlowDiagram({ steps }: { steps: string[] }) {
  ...
  return (
    <ol ref={ref} className="flow-diagram scroll-3d" aria-label="Trace execution flow">
      {steps.map((step) => (
        <li key={step}>{step}</li>
      ))}
    </ol>
...
function RecursiveWorkflowVisual() {
  ...
  return (
    <div ref={ref} className="recursive-workflow scroll-3d" aria-hidden="true">
...
function ForgeObject() {
  ...
  React.useLayoutEffect(() => {
    if (!ref.current) return;
    const plates = ref.current.querySelectorAll(".forge-plate");
    const ctx = gsap.context(() => {
      gsap.fromTo(
        ref.current,
  ...
  return (
    <div ref={ref} className="forge-object scroll-3d" aria-hidden="true">
...
function TraceEvidenceVisual() {
  ...
  React.useLayoutEffect(() => {
    if (!ref.current) return;
    const rows = ref.current.querySelectorAll(".visual-row");
    const ctx = gsap.context(() => {
      gsap.fromTo(
        ref.current,
  ...
  return (
    <div ref={ref} className="evidence-visual scroll-3d" aria-label="Trace evidence record preview">
      <div className="visual-header">
...
function OperatorCanvas() {
  ...
  React.useLayoutEffect(() => {
    if (!ref.current) return;
    const children = ref.current.querySelectorAll(".canvas-alert, .canvas-chat, .canvas-actions");
    const ctx = gsap.context(() => {
      gsap.fromTo(
        ref.current,
  ...
  return (
    <div ref={ref} className="operator-canvas scroll-3d" aria-label="Trace operator copilot preview">
